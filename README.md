...
```
#### Route Match
You put the route names in the key of the releases array. The key must match the current route name. Use a `|` to match multiple route names. The package runs through the route names. If a match is found, it stops searching. The match contains [Version Rules](#version-rules). If no Route Name match can be found, *default* will be used. That way you can update all your other endpoints. To match versions on all endpoints, you can use the *all* key.
